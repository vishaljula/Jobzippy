...
import { API_CONFIG, GOOGLE_OAUTH_CONFIG } from '../config';
...
async function exchangeCodeForTokens(code: string, codeVerifier: string): Promise<OAuthTokens> {
  const response = await fetch(`${API_CONFIG.baseUrl}/oauth/google/exchange`, {
    method: 'POST',
    headers: {
  ...
  if (!response.ok) {
    ...
    try {
      ...
      errorMessage = `${errorMessage}: ${errorBody.message || errorBody.error || response.statusText}`;
    } catch {
...
export async function refreshAccessToken(): Promise<OAuthTokens> {
  ...
  const response = await fetch(`${API_CONFIG.baseUrl}/oauth/google/refresh`, {
    method: 'POST',
    headers: {
      'Content-Type': 'application/json',
    },
